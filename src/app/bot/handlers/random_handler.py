...
async def handle_random_message(update: Update, context: ContextTypes.DEFAULT_TYPE, user_text: str) -> None:
    ...
    try:
        logger.debug(
            "RANDOM ask user=%s mode=%s topic_len=%s prev_response_id=%s",
            tg_user_id,
            mode,
            len(topic),
            last_response_id,
        ...
        logger.exception("OpenAIError on RANDOM ask; resetting continuation and retrying once (user=%s)", tg_user_id)
        try:
            await repo.set_last_response_id(tg_user_id, mode, None)
            result = await _ask(None)
        except OpenAIError:
            logger.exception("OpenAIError on RANDOM retry (user=%s)", tg_user_id)
